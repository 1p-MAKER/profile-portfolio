...
export default function PortfolioContent({ data }: { data: ContentData }) {
  ...
  const initialTab =
    data.tabs && data.tabs.length > 0 ? data.tabs[0].id : "leather";
  ...
  const getFeaturedItems = () => {
    ...
    // Leather Products
    data.leatherProducts
      ?.filter((p) => p.isFeatured && p.handle)
      .forEach((p) => {
    ...
    data.gameItems
      ?.filter((item) => item.isFeatured)
      .forEach((item) => {
        items.push({ type: "games", data: item, id: item.id });
      });
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {getFeaturedItems().map((item, index) => {
                if (item.type === "ios")
                  return (
                    <div key={`f-ios-${index}`} className="h-full">
                      <AppCard appId={item.data.id} />
                    </div>
                  );
                if (item.type === "leather")
                  return (
                    <div key={`f-leather-${index}`} className="h-full">
                      <ShopifyProductCard handle={item.data.handle} />
                    </div>
                  );
                if (item.type === "shopify" || item.type === "sns" || item.type === "games")
                  return (
                    <div key={`f-${item.type}-${index}`} className="h-full">
                      <SimpleCard
                        title={item.data.title}
                        description={item.data.description}
                        url={item.data.url}
                        category={item.type === 'games' ? 'Game' : item.data.category}
                      />
                    </div>
                  );
                if (item.type === "youtube" || item.type === "videoProduction")
                  return (
                    <div key={`f-yt-${index}`} className="h-full">
                ...
                if (item.type === "furusato") {
                  ...
                  return (
                    <div key={`f-furu-${index}`} className="h-full">
                      <FurusatoCard
                        title={item.data.title}
                        url={item.data.url}
                ...
                if (item.type === "note") {
                  return (
                    <div key={`f-note-${index}`} className="h-full">
                      <NoteCard
                        title={item.data.title}
                        url={item.data.url}
                        imageUrl={item.data.imageUrl}
                ...
                if (item.type === "audio") {
                  return (
                    <div key={`f-audio-${index}`} className="h-full">
                      <AudioCard track={item.data} />
                    </div>
                  );
                }
                    ...
                    <div key={`f-sm-${index}`} className="h-full">
                      ...
                      >
                        <div className="relative aspect-square overflow-hidden bg-stone-100">
                          ...
                          <span
                            className={`absolute top-2 right-2 text-xs font-bold px-2 py-1 rounded shadow-sm text-white ${item.data.type === "base" ? "bg-stone-800" : "bg-gradient-to-r from-purple-500 to-pink-500"}`}
                          >
      ...
      case "ai-tools":
        return (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.aiToolItems &&
                data.aiToolItems.map((item, index) => (
                  <div key={index} className="h-full">
                    <AiToolCard
                      title={item.title}
            ...
            {(!data.aiToolItems || data.aiToolItems.length === 0) && (
              <p className="text-center text-stone-500 py-12">
                まだAIツールが登録されていません。
              </p>
            )}
      ...
      case "videoProduction":
        return (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.videoProductionVideos &&
                data.videoProductionVideos.map((video) => (
                  <div key={video.id} className="h-full">
                    <div className="bg-white dark:bg-stone-800 rounded-2xl overflow-hidden shadow-sm border border-stone-200 dark:border-stone-700">
                      <YouTubeEmbed url={video.url} />
                      <div className="p-4">
            ...
            {(!data.videoProductionVideos ||
              data.videoProductionVideos.length === 0) && (
                <p className="text-center text-stone-500 py-12">
                  まだ動画が登録されていません。
                </p>
              )}
      ...
      case "note":
        return (
          <>
            ...
            {(!data.noteItems || data.noteItems.length === 0) && (
              <p className="text-center text-stone-500 py-12">
                まだ記事が登録されていません。
              </p>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.brainItems &&
                data.brainItems.map((item, index) => (
                  <div key={index} className="h-full">
                    <NoteCard
                      title={item.title}
                      url={item.url}
                      siteName={item.siteName || "Brain"}
                    />
                  </div>
                ))}
            </div>
            {(!data.brainItems || data.brainItems.length === 0) && (
              <p className="text-center text-stone-500 py-12">
                まだBrain記事が登録されていません。
              </p>
            )}
                ...
                data.tiktokItems.map((item) => (
                  <div key={item.id} className="space-y-2">
                    ...
                    <div className="flex items-center gap-1">
                      <span
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-white ${item.platform === "instagram" ? "bg-gradient-to-r from-purple-500 to-pink-500" : "bg-black"}`}
                      >
                        {item.platform === "instagram" ? "IG" : "チャレンジ"}
                      </span>
                      <h3 className="text-xs font-bold text-stone-700 line-clamp-2">
                        {item.title}
                      </h3>
                    </div>
                  </div>
        ...
        return (
          <>
            ...
            <div className="flex flex-wrap justify-center gap-6">
              {data.officeItems &&
                data.officeItems.map((item, index) => (
                  <div
                    key={index}
                    className="w-full md:w-[350px] bg-white dark:bg-stone-800 rounded-2xl overflow-hidden shadow-sm border border-stone-200 dark:border-stone-700 p-6 flex flex-col"
                  >
                    {item.imageUrl && (
                    ...
                    <h3 className="text-xl font-bold mb-2 text-stone-900 dark:text-stone-100 text-center">
                      {item.title || "無題のメニュー"}
                    </h3>
                    {item.description && (
                      <p className="text-sm text-stone-600 dark:text-stone-400 mb-6 flex-grow whitespace-pre-wrap">
                        {item.description}
                      </p>
                    )}
            ...
            {(!data.officeItems || data.officeItems.length === 0) && (
              <p className="text-center text-stone-500 py-12">
                現在販売中の商品はありません。
              </p>
            )}
  ...
  return (
    <>
          ...
          <div className="flex justify-start md:justify-center gap-2 md:gap-6 min-w-max pb-px">
            {normalizedTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => {
                  const newIndex = normalizedTabs.findIndex(
                    (t) => t.id === tab.id,
                  );
      ...
      <main
        className="flex-grow container mx-auto px-4 py-8 overflow-hidden touch-pan-y min-h-screen"
        {...bind()}
      >
